            ...
            <div className="min-h-screen flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                        ...
                        {authMode === 'register' && registrationStep === 2 && renderRegistrationStep2()}
                        <div className="flex justify-between space-x-4">
                            ...
                            <button
                                type={authMode === 'login' || registrationStep === 2 ? 'submit' : 'button'}
                                onClick={authMode === 'register' && registrationStep === 1 ? handleNext : undefined}
                                className={`${authMode === 'register' && registrationStep === 1 ? 'flex-1' : 'w-full'} inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition duration-150 ease-in-out`}
                            >
                                {authMode === 'login' ? 'Sign In' : (registrationStep === 1 ? 'Next' : 'Complete Registration')}
                                {authMode === 'register' && registrationStep === 1 && <FaArrowRight className="ml-2" />}
                            </button>
                        </div>
                    </form>
                </div>
